...
from .circle_fit import run_circle_fit
...
class FitParameters:
    ...
    # Circle-fit fields — NaN when run_circle_fit=False or the fit failed
    Q_loaded: float = float("nan")
    """Loaded quality factor Q_L from circle fit."""
    Q_internal: float = float("nan")
    """Internal quality factor Q_i from circle fit."""
    Q_external: float = float("nan")
    """External/coupling quality factor Q_e from circle fit."""
    kappa_Hz: float = float("nan")
    """Total linewidth κ/2π [Hz] from circle fit."""
    phi0_circlefit: float = float("nan")
    """Impedance mismatch angle φ₀ [rad] from circle fit."""
    delay_ns: float = float("nan")
    """Cable delay [ns] from circle fit."""
...
def log_fitted_results(fit_results: Dict, log_callable=None):
    ...
    for q in fit_results.keys():
        ...
        # Log circle-fit Q values if available
        Q_L = fit_results[q].get("Q_loaded", float("nan"))
        if np.isfinite(Q_L):
            Q_i = fit_results[q].get("Q_internal", float("nan"))
            Q_e = fit_results[q].get("Q_external", float("nan"))
            kappa = fit_results[q].get("kappa_Hz", float("nan"))
            log_callable(
                f"\tCircle fit: Q_L={Q_L:.0f}  Q_i={Q_i:.0f}  Q_e={Q_e:.0f}"
                f"  κ/2π={kappa*1e-3:.1f} kHz\n"
            )
...
def fit_raw_data(ds: xr.Dataset, node: QualibrationNode) -> Tuple[xr.Dataset, dict[str, FitParameters]]:
    ...
    When node.parameters.run_circle_fit is True, also runs the Probst circle-fit
    pipeline and populates the Q_loaded / Q_internal / Q_external / kappa_Hz /
    phi0_circlefit / delay_ns fields of each FitParameters entry.
    """
    fit_results = peaks_dips(ds.IQ_abs, "detuning")
    fit_data, fit_results = _extract_relevant_fit_parameters(fit_results, ds, node)

    if getattr(node.parameters, "run_circle_fit", False):
        geometry = getattr(node.parameters, "circle_fit_geometry", "transmission")
        for q in node.namespace["qubits"]:
            try:
                freq_Hz = ds.detuning.values + q.resonator.RF_frequency
                S21 = (ds["I"].sel(qubit=q.name).values
                       + 1j * ds["Q"].sel(qubit=q.name).values)
                cf = run_circle_fit(freq_Hz, S21, geometry=geometry)
                if cf.success:
                    r = fit_results[q.name]
                    r.Q_loaded      = cf.Q_loaded
                    r.Q_internal    = cf.Q_internal
                    r.Q_external    = cf.Q_external
                    r.kappa_Hz      = cf.kappa_Hz
                    r.phi0_circlefit = cf.phi0
                    r.delay_ns      = cf.delay_ns
                    node.namespace.setdefault("circle_fit_results", {})[q.name] = cf
            except Exception as exc:
                logging.getLogger(__name__).warning(
                    "Circle fit failed for %s: %s", q.name, exc
                )
