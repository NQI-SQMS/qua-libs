...
def plot_iq_abs_heatmap(ds: xr.Dataset, qubits, log_scale: bool = False):
    """Plot the raw spectroscopy response vs (time, frequency) as a pcolormesh per qubit.

    Uses ``IQ_abs`` when available (IQ acquisition); falls back to ``state``
    when the node was run with state discrimination. Returns a single
    matplotlib Figure, or ``None`` if neither signal variable is present.
    """
...
def plot_phase_heatmap(ds: xr.Dataset, qubits):
    ...
    Returns a single matplotlib Figure, or ``None`` when the dataset has no
    ``phase`` variable (e.g. state-discrimination runs, which only record state).
    """
