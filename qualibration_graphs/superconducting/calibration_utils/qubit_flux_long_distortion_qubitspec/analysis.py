...
def _seed_amplitudes(
    ...
    Finite-pulse model (``t_pulse_ns`` given, Aggarwal et al. arXiv:2503.08645 Appendix H, Eq. (H1),
...
def _fit_once(
    ...
    attenuation factor ``(1 - exp(-T_pulse/tau_i))`` (Aggarwal et al. arXiv:2503.08645
    Appendix H, Eq. (H1)). The returned ``a_i`` are the true step-response amplitudes (i.e.
...
def multi_exp_fit_global(
    ...
    pipeline 19a which directly samples the step rise):
        ``y(t) = a_dc + Σ a_i exp(-t/tau_i)``     [Hellings et al. arXiv:2503.04610 Eq. (1); for t>0, u(t)=1]
    ...
    duration ``T_pulse = t_pulse_ns``):
        ``y(t) = a_dc + Σ a_i (1 - exp(-T_pulse/tau_i)) exp(-t/tau_i)``
                                            [Aggarwal et al. arXiv:2503.08645 Appendix H, Eq. (H1)]
...
def _load_dispersion_curve(
    ...
    * **Legacy** coupler-flux spectroscopy: ``coupler_flux`` + ``frequency_var`` indexed by ``qubit_pair``.
    ...
    try:
        ...
        # Node-10 layout (qubit-spectroscopy-vs-coupler-flux): ``flux_bias`` dim,
        # ``peak_frequency`` / ``abs_peak_frequency`` vars, and a ``qubit`` coord that
        # holds qubit-pair names (e.g. "q0-1") with the measured qubit name in a separate
        # ``measured_qubit_name`` coord.  Resolve the correct coordinate label before
        # selecting (selecting blindly by qubit.name raises "not all values found in index").
        if "coupler_flux" not in ds_fit and "flux_bias" in ds_fit.dims:
            var = (
                frequency_var
                if frequency_var in ds_fit.data_vars
                else ("peak_frequency" if "peak_frequency" in ds_fit.data_vars else None)
            )
            if var is not None and ("qubit" in ds_fit.dims or "qubit" in ds_fit.coords):
                qvals = [str(v) for v in np.atleast_1d(ds_fit.qubit.values)]
                key = None
                if getattr(qubit, "name", None) in qvals:
                    key = qubit.name
                else:
                    for pair_name, pr in node.machine.qubit_pairs.items():
                        if pr.coupler.name == coupler.name and str(pair_name) in qvals:
                            key = str(pair_name)
                            break
                    if key is None and "measured_qubit_name" in ds_fit.coords:
                        matches = [
                            qv
                            for qv, m in zip(qvals, np.atleast_1d(ds_fit.measured_qubit_name.values))
                            if str(m) == qubit.name
                        ]
                        if len(matches) == 1:
                            key = matches[0]
                if key is not None:
                    flux_bias = np.asarray(ds_fit.flux_bias.values)
                    frequency = np.asarray(ds_fit[var].sel(qubit=key).values)
                    finite = np.isfinite(frequency)
                    if finite.sum() >= 2:
                        return (flux_bias[finite], frequency[finite])
                    return (flux_bias, frequency)
            print(
                f"Could not resolve node-10 dispersion curve for {qubit.name}/{coupler.name} "
                f"in load_id {load_id}"
            )
            return None
