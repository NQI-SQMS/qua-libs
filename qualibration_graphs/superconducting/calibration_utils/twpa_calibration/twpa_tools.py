...
def dBm(full_scale_power_dbm, daps):
    v = (
        np.sqrt((2 * 50 * 10 ** (full_scale_power_dbm / 10)) / 1000) * daps * 1
    )  # 1 : twpa readout amplitude  #opx1000 documentation
    p_w = (v**2) / 50
...
def min_dsnr(qubits, dsnr_avg, snr_off, pgg, dfps, daps):
    ...
    if qubits[0].T1 is None:
        print("T1 is not defined, using default T1=50us")
        t1 = 50
    else:
        t1 = qubits[0].T1 * 1e6  # in us
    f = fidelity(qubits[0].resonator.operations["readout"].length * 1e-3, t1, snr_off * twpa_fct, pgg)
...
def min_gain(qubits, twpas):
    ...
    ps_on = (
        dBm(
            qubits[0].resonator.opx_output.full_scale_power_dbm,
            a_ro * qubits[0].resonator.operations["readout"].amplitude,
        )
        - 60
        - 6
        - 5
...
def multiplexed_optimizer(qubit, Gain, dsnr, qubits):  # qubit = worst snr qubit
    ...
    for i in range(len(qubits)):
        print(
            f"qB{i + 1}:dSNR:{np.round(dsnr[i][idx[0]][idx[1]][0], 2)}dB, gain:{np.round(Gain[i][idx[0]][idx[1]][0], 2)}dB"
        )
    return idx
...
def delta_s(RF_freq, ds_off_s, ds_on_s, qubits, dfps, daps):
    f_stack = np.stack(
        [RF_freq[i] * 1e-9 for i in range(len(qubits)) for j in range(len(dfps)) for k in range(len(daps))], axis=0
    )
