"""Analysis utilities for qubit ge spectroscopy at Fock |N⟩."""
import numpy as np
import xarray as xr
...
def fit_raw_data(ds: xr.Dataset, frequency_span_in_mhz: float, use_state_discrimination: bool, chi_k_estimate: float, chi_linear: float):
    """Fit a Lorentzian peak to ge state-vs-detuning data at a given Fock level.

    Returns (ds_fit, fit_results) where fit_results maps qubit name → dict with
    keys: frequency_hz, delta_f_focka_hz, fwhm_hz, success.

    delta_f_focka_hz = chi_k_estimate + f0_fit - chi_linear, i.e. the deviation
    of the measured dispersive shift from the linear prediction (k+1) × chi.
    """
    ...
    for q in ds.qubit.values:
        ...
        try:
            ...
            fit_results[q] = {
                "frequency_hz": f0_fit,
                "delta_f_focka_hz": float(chi_k_estimate) + f0_fit - chi_linear,
