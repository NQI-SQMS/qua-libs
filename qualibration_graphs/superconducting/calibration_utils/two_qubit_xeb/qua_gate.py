"""QUA gate definitions for two-qubit XEB."""
...
class QUAGate:
    ...
    def __init__(
        ...
        gate_macro: Optional[Callable[[Union[FluxTunableTransmon, FluxTunableTransmonPair]], None]] = None,
    ...
    @property
    def name(self):
        """Return gate name from label or gate.name."""
        return self.gate.name if self.gate.label is None else self.gate.label

    @property
    def gate(self) -> Gate:
        """Return the underlying Qiskit Gate."""
        return self._gate
