"""QUA gate set definitions for two-qubit XEB."""
...
def play_sq_gate_macro(
...
) -> Callable[[FluxTunableTransmon], None]:
    """
    ...
    def play_sq_gate(qubit: FluxTunableTransmon):
        qubit.xy.play(
            baseline_gate_name,
...
def play_virtual_t_gate(qubit: FluxTunableTransmon):
    """
...
class QUAGateSet(dict):
    ...
    Args:
        gate_set: Gate Set for XEB. Two native gate sets are available through 'sw' and 't'.
            Those sets contain X/2, Y/2, and SW gates, or X/2, Y/2, and T gates respectively.
            In "sw", all gates are playable via amplitude matrix modulation of a baseline X/2 pulse.
            In "t", the T gate is a virtual pi/4 rotation around Z.
            Alternatively, a custom gate set can be provided as a dict of QUAGate objects
            with consecutive integer keys starting from 0.
        baseline_gate_name: Name of the baseline gate implementing a pi/2 rotation around x.
    """
