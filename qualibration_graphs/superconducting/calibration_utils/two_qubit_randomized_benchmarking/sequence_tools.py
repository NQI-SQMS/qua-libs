...
def split_ops(ops: List[Op]) -> List[List[Op]]:
    ...
    while i < n:
        ...
        if tgt == "01":
            ...
            groups.append([("I", "0"), ("I", "1"), ("CNOT", "01"), ("I", "0"), ("I", "1")])
            i += 1
        elif tgt == "0":
            if i + 1 >= n:
                raise ValueError(f"Expected ('*','1') after index {i}, got end of list.")
            _, nxt = ops[i + 1]
            if nxt != "1":
                raise ValueError(f"Expected ('*','1') after ('{gate}','0') at index {i}, got ('*','{nxt}').")
            groups.append([ops[i], ops[i + 1]])
            i += 2
        elif tgt == "1":
            raise ValueError(f"Unexpected single on '1' at index {i}; singles must be '0' then '1'.")
        else:
...
#         return instruct_integers
#     sequence_ints = np.random.randint(len(instruct_list), size=depth).tolist()
...
#     for c_ in range(len(instruct_list)):
...
def multiply_unitaries(unitaries):
    unitary = np.eye(4)
    for U in unitaries:
        unitary = unitary @ U
    return unitary

    # this is slow for a large list of unitaries
    # if len(unitaries) == 1:
    #     return unitaries[0]
    # else:
    #     return np.linalg.multi_dot(unitaries[::-1])


def generate_sequence_list_interleaved(depth, interleaved_instruct=None, exclude_cnot=False):
    """
    for a given depth, generate ONE random sequence containing 2q clifford gates at a certain depth
    interleaved by CNOT
    return:
        instruct_integers: a list of integers each corresponding to a specific pulse (see "run=two-qubit-rb-CR-CNOT.py")
    """
    instruct_list, unitary_list = load_pkls(exclude_cnot)

    # convert the interleaved instruction into unitary
    if interleaved_instruct:
        ...
        interleaved_unitaries = [unitary_list[i] for i in interleaved_idx]
        interleaved_unitary = multiply_unitaries(interleaved_unitaries)

    # for depth 0, just return identity
    if depth == 0:
        instruct = []
        instruct.extend(instruct_list[0])
        instruct_integers = instruct_to_integer(instruct)
    ...
    # generate random sequence and interleave if needed
    sequence_ints = np.random.randint(len(instruct_list), size=depth).tolist()
    instruct = []
    unitaries = []
    for d_ in range(depth):
    ...
    unitary = multiply_unitaries(unitaries)

    # find the inverse for the unitary of the sequence
    thresh = 3.99  # 3.99 rather than 4 to account for any floating point error; ideally should be 4
    if len(unitaries) == 1:
        unitary = unitaries[0]
    else:
        unitary = np.linalg.multi_dot(unitaries[::-1])
    inverse_found = False
    for c_ in range(len(instruct_list)):
        # if np.allclose(unitary @ unitary_list[c_], np.eye(4), rtol=0.001):
        if abs((unitary @ unitary_list[c_]).trace()) > 3.99:
            # set a bar for judging whether circuit[c_] is the inverse gate
            # 3.99 rather than 4 to account for any floating point error; ideally should be 4
            inverse_found = True
            break
    assert inverse_found, "No inverse clifford found!"
    sequence_ints.append(c_)
    instruct.extend(instruct_list[c_])
    instruct_integers = instruct_to_integer(instruct)

    return instruct_integers, instruct
...
def pre_generate_sequence_interleaved(number_of_sequences, depth_list, interleaved_instruct=None, exclude_cnot=False):
    sequence_list = []
    len_list = []
    full_list = []
    for ns_ in tqdm(range(number_of_sequences), desc="Generating sequences"):
        for depth in depth_list:
            if depth == 0:
                # put identity when depth is 0
                sequence_list.extend([0])
                len_list.append(1)
                full_list.append([("I", "0"), ("I", "1")])
            else:
                new_list, instruct_list = generate_sequence_list_interleaved(
                    depth, interleaved_instruct, exclude_cnot=exclude_cnot
                )
                sequence_list.extend(new_list)
                len_list.append(len(new_list))
                full_list.append(instruct_list)

    return sequence_list, len_list, full_list


def load_pkls(exclude_cnot=False):
    # Always resolve PKLs relative to this module so direct script/test execution works.
    ...
    instruct_list_copy = instruct_list.copy()
    unitary_list_copy = unitary_list.copy()
    if exclude_cnot:
        instruct_list = []
        unitary_list = []
        for instruct, unitary in zip(instruct_list_copy, unitary_list_copy):
            if ("CNOT", "01") not in instruct:
