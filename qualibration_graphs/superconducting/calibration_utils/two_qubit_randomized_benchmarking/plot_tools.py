...
def plot_spectrum(
    ...
    fig = px.line(
        x=freq_ax[1:], y=signal_fft[1:], labels={"x": "frequency [MHz]", "y": "power"}
    )
...
class QubitPairGrid:
    ...
    which should be of the form: 'q-i,j-q-n,m' where i,j and n,m are
    ...
    def __init__(
        ...
        # Process the grid names and extract indices
        if len(grid_names) > 1:
            qubit_indices = [
                (
        ...
        # Create axes grid as a 2D array if required.
        if shape == (1, 1):
            axes_grid = np.array(all_axes)
        else:
            axes_grid = all_axes.reshape(shape)

        # Prepare lists for the axes that will be used and their corresponding qubit names.
        axes = []
        qubit_names = []

        for row, axis_row in enumerate(axes_grid):
            for col, ax in enumerate(axis_row):
                # Change here: use a matrix mapping (increasing row means lower down)
...
class QubitGrid:
    ...
    Optionally it accepts a parameter "grid_names" that specifies the position of each qubit on the grid.
    It assumes that qubit names are of the form 'q-i,j' where i and j are integers describing the row and column coordinates.
    """
    ...
    def __init__(self, ds: xr.DataArray, grid_names: list = None, size: int = 3):
        ...
        # Update extraction: treat the first element of the tuple as the row and the second as the column.
        grid_row_idxs = [idx[0] for idx in grid_indices]
        grid_col_idxs = [idx[1] for idx in grid_indices]
        min_grid_row = min(grid_row_idxs)
        min_grid_col = min(grid_col_idxs)
        shape = (
            max(grid_row_idxs) - min_grid_row + 1,
        ...
        for row, axis_row in enumerate(all_axes):
            for col, ax in enumerate(axis_row):
                # Map row index to grid row and col index to grid column
                grid_row = row + min_grid_row
                grid_col = col + min_grid_col
                if (grid_row, grid_col) in grid_indices:
                    axes.append(ax)
                    name = grid_name_mapping.get((grid_row, grid_col))
                    if name is not None:
