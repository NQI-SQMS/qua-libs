"""Plotting utilities for conditional qubit spectroscopy vs flux calibration."""
...
def plot_raw_data(ds: xr.Dataset, qubit_pairs: List, quantity: str = "IQ_abs", size: int = 8) -> Dict[str, Figure]:
    ...
    grid_control.fig.suptitle(f"Conditional qubit spectroscopy vs flux — control qubit readout ({quantity})")
    ...
    grid_target.fig.suptitle(f"Conditional qubit spectroscopy vs flux — target qubit readout ({quantity})")
