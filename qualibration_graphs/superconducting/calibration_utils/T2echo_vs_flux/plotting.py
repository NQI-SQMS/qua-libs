"""Plotting for the T2-echo-versus-flux node."""
...
def plot_raw_data_with_fit(ds: xr.Dataset, qubits: List[AnyTransmon], fits: xr.Dataset):
    """Plot the 2-D echo-decay map (idle time vs flux bias) for each qubit.

    A horizontal dashed line marks the flux bias giving the longest T2 echo.
    """
    grid = QubitGrid(ds, [q.grid_location for q in qubits])
    for ax, qubit in grid_iter(grid):
        _plot_individual_map(ax, ds, qubit, fits.sel(qubit=qubit["qubit"]))

    grid.fig.suptitle("T2 echo vs flux (decay map)")
    grid.fig.set_size_inches(15, 9)
    grid.fig.tight_layout()
    return grid.fig


def plot_t2_echo_vs_flux(ds: xr.Dataset, qubits: List[AnyTransmon], fits: xr.Dataset):
    """Plot the extracted T2 echo versus flux bias (with error bars) for each qubit."""
    grid = QubitGrid(ds, [q.grid_location for q in qubits])
    for ax, qubit in grid_iter(grid):
        _plot_individual_curve(ax, fits.sel(qubit=qubit["qubit"]), qubit)

    grid.fig.suptitle("T2 echo vs flux")
    grid.fig.set_size_inches(15, 9)
    grid.fig.tight_layout()
    return grid.fig
...
def _plot_individual_map(ax: Axes, ds: xr.Dataset, qubit: dict, fit: xr.Dataset = None):
    ...
    if two_d:
        ...
        if fit is not None:
            valid = np.isfinite(fit["T2_echo"].values)
            if valid.any():
                idx = int(np.nanargmax(np.where(valid, fit["T2_echo"].values, -np.inf)))
                best_flux = float(fit.flux_bias.values[idx])
                ax.axhline(best_flux, color="red", linestyle="--", linewidth=1, label="max T2e")
    else:
...
def _plot_individual_curve(ax: Axes, fit: xr.Dataset, qubit: dict):
    ...
    tau_us = fit["T2_echo"].values * 1e-3  # ns -> us
    err_us = fit["T2_echo_error"].values * 1e-3
