...
def fit_rabi_chevron(ds_qp, init_length, init_detuning):
    ...
    try:
        ...
        a = popt[2]
        offset = popt[3]
        # v2 fit-quality: the downstream sanity windows (length/amp ranges)
        # cannot tell a real chevron from a junk convergence that happens to
        # land inside them. Grade the 2D model against the data: R² plus the
        # fitted oscillation amplitude in units of the residual scatter.
        pred = rabi_chevron_model(fdata, *popt)
        resid = tdata - pred
        ss_tot = float(np.sum((tdata - np.mean(tdata)) ** 2)) + 1e-30
        r2 = 1.0 - float(np.sum(resid**2)) / ss_tot
        scatter = 1.4826 * float(np.median(np.abs(resid - np.median(resid)))) + 1e-15
        amp_snr = abs(float(a)) / scatter
        return J, f0, a, offset, r2, amp_snr
...
class FitParameters:
    ...
    fit_r2: float = float("nan")    # 2D chevron-model R² on the full map
    amp_snr: float = float("nan")   # fitted oscillation amplitude / residual scatter
...
def fit_chevron_cz(ds, dim):
    def fit_routine(ds_qp):
        ...
        try:
            ...
            f = ds_qp.detuning
            t, f = np.meshgrid(t, f)
            J, f0, a, offset, r2, amp_snr = fit_rabi_chevron(ds_qp, lengths * 2, detunings.values)

            # Check if fitting produced valid results
            if np.isnan(J) or np.isnan(f0):
                # Return default/invalid values
                return xr.DataArray([float("nan")] * 6, dims=["fit_vals"])
            ...
            return xr.DataArray([J, f0, a, offset, r2, amp_snr], dims=["fit_vals"])
        except Exception as e:
    ...
    fit_res = fit_res.assign_coords(fit_vals=("fit_vals", ["J", "f0", "a", "offset", "r2", "amp_snr"]))
    return fit_res
...
def _extract_relevant_fit_parameters(fit: xr.Dataset, node: QualibrationNode):
    ...
    for qp in fit.qubit_pair.values:
        try:
            ...
            def _fv(name):
                try:
                    return float(fit.fit.sel(qubit_pair=qp, fit_vals=name).values.item())
                except Exception:
                    return float("nan")
            r2_val = _fv("r2")
            snr_val = _fv("amp_snr")
            ...
            if np.isnan(J_val) or np.isnan(f0_val) or J_val <= 0:
                ...
                try:
                    ...
                    # v2: the fitted MODEL must actually describe the map — a
                    # junk convergence inside the sanity windows used to pass.
                    # NaN metrics (quality not computable) do not gate.
                    fit_quality_ok = (np.isnan(r2_val) or r2_val >= 0.3) and \
                                     (np.isnan(snr_val) or snr_val >= 3.0)
                    success = bool(is_length_valid and is_amp_valid and is_not_nan
                                   and fit_quality_ok)
                    if not fit_quality_ok:
                        logging.getLogger(__name__).warning(
                            "chevron_cz %s: fit rejected by quality gate (r2=%.2f, amp_snr=%.1f)",
                            qp, r2_val, snr_val)
                except Exception:
