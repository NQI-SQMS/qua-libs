...
def _relock_twin_axis(ax, ax2, center_hz: float) -> None:
...
def plot_individual_amplitude_with_fit(
    ...
    if fit is not None:
        ...
        if not np.any(np.isnan(popt)):
            ...
            # Semi-transparent FWHM band (on detuning axis)
            center_hz = full_freq_q[0] - ds_q.detuning.values[0]
