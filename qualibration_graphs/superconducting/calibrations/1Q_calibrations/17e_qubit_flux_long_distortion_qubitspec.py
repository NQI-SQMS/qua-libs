...
node = QualibrationNode[Parameters, Quam](
    name="17e_qubit_flux_long_distortion_qubitspec",
...
def plot_data(node: QualibrationNode[Parameters, Quam]):
    ...
    # Raw heatmaps are only available depending on the acquisition mode:
    # IQ_abs/phase exist for IQ runs, state exists for state-discrimination runs.
    optional_figures = {
    ...
    for key, fig in optional_figures.items():
        if fig is not None:
            figures[key] = fig
