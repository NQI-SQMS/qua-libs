...
node = QualibrationNode[Parameters, Quam](
    name="08e_readout_frequency_amplitude_optimization",
    ...
    with program() as node.namespace["qua_program"]:
        ...
        with stream_processing():
            ...
            for i in range(num_qubits):
                Ig_st[i].buffer(len(amps)).buffer(len(dfs)).buffer(n_runs).save(f"Ig{i + 1}")
                Qg_st[i].buffer(len(amps)).buffer(len(dfs)).buffer(n_runs).save(f"Qg{i + 1}")
                Ie_st[i].buffer(len(amps)).buffer(len(dfs)).buffer(n_runs).save(f"Ie{i + 1}")
                Qe_st[i].buffer(len(amps)).buffer(len(dfs)).buffer(n_runs).save(f"Qe{i + 1}")
