"""
1D Resonator Spectroscopy Calibration Node.
"""
...
from calibration_utils.resonator_spectroscopy_new import (
    Parameters,
    process_raw_dataset,
    fit_raw_data,
    log_fitted_results,
...
from calibration_utils.resonator_spectroscopy_new.escalation import (
...
node = QualibrationNode[Parameters, Quam](
    name="03_resonator_spectroscopy_single",  # Name should be unique
...
def _execute_and_fetch(node: QualibrationNode[Parameters, Quam]) -> None:
    """Connect, run the prepared QUA program, and store ds_raw (shared with retries)."""
...
def _analyse(node: QualibrationNode[Parameters, Quam]) -> None:
    """Process + fit + log + outcomes (shared between the main pass and retries)."""
...
def load_data(node: QualibrationNode[Parameters, Quam]):
    ...
    # Restore parameters that were set by the user for this re-analysis run
...
def escalate_no_dip(node: QualibrationNode[Parameters, Quam]):
