...
"""Coupler zero-interaction calibration module.

This module determines the flux bias point for tunable couplers that results in zero
effective coupling (g ≈ 0) between pairs of flux-tunable qubits.
"""
...
def update_state(node: QualibrationNode[Parameters, Quam]):
    ...
    with node.record_state_updates():
        for qp in node.namespace["qubit_pairs"]:
            ...
            # Establish and persist the CZ |20>/|02> branch so downstream CZ nodes inherit
            # it (resolve_cz_branch(qp, None) reads qp.extras["cz_branch"]). On a tunable-
            # coupler chip this coarse node is first in the CZ flow, so it plays the role
            # the chevron node plays on a fixed-coupler chip. "auto" picks the physically
            # reachable branch from the stored frequencies -- no extra measurement.
            # A new extras key is a jsonpatch "add" op which record_state_updates() drops
            # (it keeps only "replace" ops), so record it manually at the leaf, same
            # revert-then-prompt pattern as the chevron node / nodes 17/18.
            try:
                branch = str(resolve_cz_branch(qp, getattr(node.parameters, "cz_branch", "auto")).branch)
                ...
                node.log(f"{qp.name}: could not resolve CZ branch ({e}); leaving extras['cz_branch'] unchanged")
