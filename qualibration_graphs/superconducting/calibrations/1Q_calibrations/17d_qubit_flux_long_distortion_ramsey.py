...
qubit flux pulse and probing the qubit frequency at variable delay times using a
...
Workflow:
For each qubit, sweep the frame rotation and the delay time after the onset of the
qubit flux pulse.  At each delay a Ramsey sequence (x90 – wait – frame_rotation – x90)
...
State update (optional): write the fitted exponential filter to the qubit's
z.opx_output.exponential_filter.

Prerequisites
- A valid rotation angle and threshold if using state discrimination.
- Calibrated XYZ delay.
- Calibrated x90 pulse.
- Qubit spectroscopy vs flux (node 09) with freq_vs_flux_01_quad_term stored for each
  measured qubit.
...
- If `update_state=True` and fits succeed, the script updates the qubit at
  `z.opx_output.exponential_filter` with the cascade coefficients `(A_c, tau_c)`.
REMINDER: Adding digital filters will add a global delay — need to recalibrate IQ blobs
(rotation_angle & ge_threshold) and XYZ delay.

Ref: https://arxiv.org/abs/2111.13504
"""

node = QualibrationNode[Parameters, Quam](
    name="19b_qubit_flux_long_distortion_ramsey",
...
def plot_data(node: QualibrationNode[Parameters, Quam]):
    ...
    # If fit_raw_data flagged branch-aliasing risk (phase swing approaching one
    # 2pi window), stamp a visible warning on the flux-response figures — the
    # terminal warning is easy to miss.
    annotate_branch_risk(fig_proc_ln, ds)
    annotate_branch_risk(fig_proc_log, ds)
