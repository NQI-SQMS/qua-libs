"""Resonator spectroscopy versus coupler flux calibration (``_new``).

Same experiment as ``1Q_07_resonator_spectroscopy_vs_coupler_flux`` but using the
self-contained ``resonator_spectroscopy_vs_coupler_flux_new`` analysis/plotting,
which keys everything by the **unique qubit-pair name**.  This fixes the crash
that occurred when two qubit pairs share the same measured (control/target)
qubit (e.g. ``q4-5`` and ``q4-7`` both measure ``q4``): the old code renamed the
``qubit_pair`` dimension to the measured-qubit name, producing a duplicate
``qubit`` label, so ``fit_raw_data``'s ``.sel(qubit="q4")`` returned a 2-D array
and ``_fit_sinusoid`` crashed at ``flux = flux[finite]``
...
import numpy as np
import xarray as xr
from calibration_utils.resonator_spectroscopy_vs_coupler_flux_new import (
    Parameters,
    process_raw_dataset,
    fit_raw_data,
    log_fitted_results,
...
node = QualibrationNode[Parameters, Quam](
    name="07_resonator_spectroscopy_vs_coupler_flux",  # Name should be unique
...
def custom_param(node: QualibrationNode[Parameters, Quam]):
    ...
    # node.parameters.qubit_pairs = ["qA1-A2"]
    pass
...
def create_qua_program(node: QualibrationNode[Parameters, Quam]):
    ...
    with program() as node.namespace["qua_program"]:
        # Declare one I/Q pair PER QUBIT PAIR (the loops below index by the pair
        # index `ii` and the stream uses range(num_qubit_pairs)). The default
        # sizes to len(machine.qubits), which overruns when there are more pairs
        # than qubits (e.g. qubit_pairs=None selects all pairs).
...
def execute_qua_program(node: QualibrationNode[Parameters, Quam]):
    ...
    # Rename qubit_pair dimension to qubit for the analysis functions, but KEEP the
    # unique qubit-pair-name labels (do NOT relabel to the measured-qubit name, which
    # collides when two pairs share a measured qubit).
...
def load_data(node: QualibrationNode[Parameters, Quam]):
    ...
    # Rename qubit_pair dimension to qubit, keeping the unique pair-name labels.
...
def analyse_data(node: QualibrationNode[Parameters, Quam]):
    ...
    # Outcomes keyed by qubit-pair name (fit results are now keyed by pair name)
    node.outcomes = {
        qp.name: ("successful" if node.results["fit_results"].get(qp.name, {}).get("success", False) else "failed")
...
def plot_data(node: QualibrationNode[Parameters, Quam]):
    """Plot the raw and fitted data, one panel per qubit pair."""
...
def update_state(node: QualibrationNode[Parameters, Quam]):
    ...
    with node.record_state_updates():
        for qp in node.namespace["qubit_pairs"]:
            ...
            # Access the measured qubit if needed:
