...
from calibration_utils.fNgN1_ge_ramsey import Parameters
...
# %% {Node initialisation}
description = """
        ge RAMSEY AT FOCK |n⟩ — precise calibration of delta_f_focka
...
delta_f_focka value measured by node 26e (ge spectroscopy) with much higher precision.

Both signs of the artificial detuning ±δ are swept so the frequency correction is
unambiguous:
    f_obs±  =  |f_drive - f_ge_focka ± δ|
    correction  =  (f_obs+ - f_obs-) / 2

At fock_level=0: pair.chi -= correction  (chi is the reference; delta_f_focka stays 0).
At fock_level>0: delta_f_focka -= correction.
...
Sequence:
  ...
  2. Set ge drive to dispersive-shifted frequency (_ge_if_at_fock).
  3. π/2 ge pulse (x180 at half amplitude).
  4. Wait τ + virtual frame rotation (±δ).
  5. π/2 ge pulse.
  6. Measure qubit state.

Prerequisites:
    - Calibrated delta_f_focka (node 26e, fock_level=k) — provides the centre frequency.
    - Calibrated ge π-pulse (nodes 03a, 04b).
...
def plot_data(node: QualibrationNode[Parameters, Quam]):
    ...
    plt.show()
    node.results["figures"] = {f"f{k}g{k+1}_ge_ramsey": fig}
    ...
    with node.record_state_updates():
        for q_name, res in node.results["fit_results"].items():
            ...
            if k == 0:
                ...
                pair.chi = chi - res["freq_offset"]
                pair.transitions[tr_key].delta_f_focka = 0.0
            else:
                current_delta = pair.transitions[tr_key].delta_f_focka
                if current_delta is None:
                    current_delta = 0.0
                pair.transitions[tr_key].delta_f_focka = current_delta - res["freq_offset"]
            break  # delta_f_focka is per pair/mode, not per qubit
