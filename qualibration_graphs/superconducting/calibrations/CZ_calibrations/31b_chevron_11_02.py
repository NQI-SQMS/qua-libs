...
import numpy as np
import xarray as xr
from calibration_utils.chevron_cz_v2 import (
    Parameters,
    baked_waveform,
    fit_raw_data,
    log_fitted_results,
...
NOTE: Despite the "1102" name, this node calibrates the |11> <-> |20> chevron
(the control qubit is flux-tuned and transiently occupies |2>; control uses GEF readout,
target uses g-e readout). The flux-pulse detuning therefore uses the CONTROL anharmonicity.
...
node = QualibrationNode[Parameters, Quam](
    name="31_chevron_11_02",  # Name should be unique
...
def create_qua_program(node: QualibrationNode[Parameters, Quam]):
    ...
    # define the amplitudes for the flux pulses and resolve the |20>/|02> branch roles per pair.
    # Both branches flux-move the CONTROL qubit; the branch only sets the anharmonicity term in the
    # detuning and which qubit transiently reaches |2> (and is therefore GEF-read).
    pulse_amplitudes = {}
    branch_specs = {}
    for qp in qubit_pairs:
        spec = resolve_cz_branch(qp, node.parameters.cz_branch)
        branch_specs[qp.name] = spec
        pulse_amplitudes[qp.name] = spec.pulse_amplitude

    node.namespace["pulse_amplitudes"] = pulse_amplitudes
    node.namespace["branch_specs"] = branch_specs
                ...
                with for_(*from_array(a, amplitudes)):
                    ...
                    with for_(*from_array(t, times_cycles)):
                        for ii, qp in multiplexed_qubit_pairs.items():
                            ...
                            # Branch roles: *_control streams = the |2>-excited qubit, *_target streams =
                            # the other (chevron-signal) qubit. For branch "20" excited=control (== legacy).
...
def update_state(node: QualibrationNode[Parameters, Quam]):
    ...
    for qp in node.namespace["qubit_pairs"]:
        if hasattr(qp.macros, "cz_unipolar"):
            continue
        node.log(f"Creating CZ Unipolar gate macro for {qp.name}")
        cz_pulse = SquarePulse(
            length=100, amplitude=0.5, id="cz_unipolar_pulse"
        )
        cz = CZGate(flux_pulse_qubit=cz_pulse)
        node.machine.qubit_pairs[qp.name].macros["cz_unipolar"] = cz
        pulse_length = (
            node.machine.qubit_pairs[qp.name].macros["cz_unipolar"].flux_pulse_qubit.get_reference() + "/length"
        )
        pulse_amp = (
            node.machine.qubit_pairs[qp.name].macros["cz_unipolar"].flux_pulse_qubit.get_reference() + "/amplitude"
        )
        pulse_name = node.machine.qubit_pairs[qp.name].macros["cz_unipolar"].flux_pulse_qubit_label
        control_qb = node.machine.qubit_pairs[qp.name].qubit_control
        control_qb.z.operations[pulse_name] = SquarePulse(
            length=100, amplitude=0.25
        )
        ...
        if hasattr(qp.macros, "cz_flattop"):
            continue
        node.log(f"Creating CZ Flattop gate macro for {qp.name}")
        cz_pulse = FlatTopGaussianPulse(
            length=100, amplitude=0.5, flat_length=50, id="cz_flattop_pulse"
        )
        cz = CZGate(flux_pulse_qubit=cz_pulse)
        node.machine.qubit_pairs[qp.name].macros["cz_flattop"] = cz
        pulse_length = (
            node.machine.qubit_pairs[qp.name].macros["cz_flattop"].flux_pulse_qubit.get_reference() + "/length"
        )
        flat_length = (
            node.machine.qubit_pairs[qp.name].macros["cz_flattop"].flux_pulse_qubit.get_reference() + "/flat_length"
        )
        pulse_amp = (
            node.machine.qubit_pairs[qp.name].macros["cz_flattop"].flux_pulse_qubit.get_reference() + "/amplitude"
        )
        pulse_name = node.machine.qubit_pairs[qp.name].macros["cz_flattop"].flux_pulse_qubit_label
        control_qb = node.machine.qubit_pairs[qp.name].qubit_control
        control_qb.z.operations[pulse_name] = FlatTopGaussianPulse(
            length=100, amplitude=0.5, flat_length=50
        )
        control_qb.z.operations[pulse_name].length = pulse_length
        control_qb.z.operations[pulse_name].amplitude = pulse_amp
    ...
    with node.record_state_updates():
        for qp in node.namespace["qubit_pairs"]:
            ...
            print(node.results["fit_results"][qp.name]["cz_len"])
            qp.macros["cz_flattop"].flux_pulse_qubit.flat_length = int(
                np.ceil(node.results["fit_results"][qp.name]["cz_len"] / 4) * 4
            )
            qp.macros["cz_flattop"].flux_pulse_qubit.length = (
                int(np.ceil(node.results["fit_results"][qp.name]["cz_len"] / 4) * 4) + 20
            )
            # Persist the calibrated branch on the pair so downstream CZ nodes inherit it
            # (resolve_cz_branch(qp, None) reads qp.extras["cz_branch"]).
            # Adding a NEW dict key produces a jsonpatch "add" op, which record_state_updates()
            # drops (it only keeps "replace" ops), so the change is neither shown in the GUI nor
            # saved. Record it manually at the leaf level, same revert-then-prompt pattern as
            # nodes 17/18.
            branch = str(node.namespace["branch_specs"][qp.name].branch)
            ref = f"#/qubit_pairs/{qp.name}/extras/cz_branch"
            old = qp.extras.get("cz_branch", None)
            qp.extras = {**qp.extras, "cz_branch": branch}
            attr_key = update_machine_attribute(node.machine, ref, old)
            record_state_update(node, ref, str(attr_key), old, branch)
