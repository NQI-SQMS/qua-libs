...
from calibration_utils.snz_b_over_a_2 import decompose_t_phi_eff, snz_factory
from calibration_utils.snz_conditional_phase_2 import (
    Parameters,
    fit_raw_data,
    log_fitted_results,
...
This is the analysis+update variant of 39: it writes the pi / low-leakage
operating point to the cz_SNZ macro as a (non-amplified) CZ calibration. Run
39b afterwards for the amplified, high-precision refinement.

State update (cz_SNZ macro only):
    - qp.macros["cz_SNZ"].flux_pulse_qubit.amplitude   (abs V at pi / low-leakage)
    - qp.macros["cz_SNZ"].flux_pulse_qubit.t_phi_eff
    - qp.macros["cz_SNZ"].flux_pulse_qubit.flat_length (the length used in the scan)
"""

node = QualibrationNode[Parameters, Quam](
    name="39_2_snz_conditional_phase",
...
def update_state(node: QualibrationNode[Parameters, Quam]):
    """Write the pi / low-leakage operating point to cz_SNZ (amplitude + t_phi_eff + flat_length).

    39_2 measures the conditional phase, so the written point is a genuine
    (non-amplified) CZ calibration: among the low-leakage region it is the one
    whose conditional phase is closest to pi (0.5 in 2pi units). Run 39b after for
    the amplified high-precision refinement. Pairs without a cz_SNZ macro or with a
    failed fit are skipped. ``operation`` only seeds the baking nominal A / length;
    results are always written to cz_SNZ.
    """
    operation = node.parameters.operation
    fit_results = node.results["fit_results"]
    with node.record_state_updates():
        for qp in node.namespace["qubit_pairs"]:
            ...
            node.log(
                f"{qp.name}: cz_SNZ written -- amplitude={amp_abs:.6f} V, t_phi_eff={tpe_opt:.4f} ns, "
                f"flat_length={int(scanned_length)} (cond.phase={fr['optimal_phase']:.4f} "
                f"[target 0.5], leakage={fr['optimal_leakage']:.4f})."
            )
