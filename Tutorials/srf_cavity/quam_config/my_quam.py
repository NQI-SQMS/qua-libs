...
class TemporaryCalibrationData(QuamBase):
    ...
    # ── Bayesian optimiser suggestions ───────────────────────────────────────
    bo_suggested: Optional[Dict[str, Any]] = None
    """Per-node BO parameter suggestions.  Keyed by node_key (e.g. '03c', '04b').
    Written by BONodeController.should_retry; read by the node's create_qua_program.
    Cleared on convergence."""
...
class Quam(FixedFrequencyQuam):
    """QUAM for a fixed-frequency transmon coupled to SRF cavities on OPX+/Octave or OPX1000.
